...
- Grok API keys are typically 20+ characters (no specific prefix required)
...
- **📱 Share Roast** - Web Share API (native share dialog)
- **🔗 Copy Link** - Copy debate URL to clipboard
- **📹 TikTok** - Copy formatted text for TikTok
- **🐦 X Post** - Open Twitter/X with pre-filled tweet
...
After analysis, users see:
- **📝 Notes** - Web Share API → Send to Notes app
- **🟢 Evernote** - Deep link to Evernote (`evernote://`)
- **🎯 Notion** - Copy formatted text + attempt Notion deep link
- **💾 Save Local** - Download as `.txt` file
...
│   └── src/
│       ├── components/
│       │   ├── SocialShareBar.jsx  # NEW: Petty Mode sharing
│       │   └── ExportBar.jsx       # NEW: Productive Mode export
│       ├── lib/
│       │   ├── aiService.js
│       │   ├── debateUtils.js
│       │   └── security.js
...
└── package.json             # Added react-share
```
...
- Ensure keys have correct format (Grok: 20+ chars, Claude: `sk-ant-*`)
